...
def read_egk():
    ...
    xml_dict = xmltodict.parse(patient_data_xml)
    versicherter_dict = xml_dict['UC_PersoenlicheVersichertendatenXML']['Versicherter']
    geburtsdatum = versicherter_dict['Person']['Geburtsdatum']
    return {
        'VersichertenID': versicherter_dict['Versicherten_ID'],
        'Vorname': versicherter_dict['Person']['Vorname'],
        'Nachname': versicherter_dict['Person']['Nachname'],
        'Geschlecht': versicherter_dict['Person']['Geschlecht'].lower(),
        'Geburtsdatum': f'{geburtsdatum[6:]}.{geburtsdatum[4:6]}.{geburtsdatum[:4]}',
        'PLZ': versicherter_dict['Person']['StrassenAdresse']['Postleitzahl'],
        'Ort': versicherter_dict['Person']['StrassenAdresse']['Ort'],
        'Strasse': versicherter_dict['Person']['StrassenAdresse']['Strasse'],
        'Hausnummer': versicherter_dict['Person']['StrassenAdresse']['Hausnummer'],
        }
    return ['Person']
